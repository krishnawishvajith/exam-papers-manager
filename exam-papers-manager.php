...
class ExamPapersManager
{
    ...
    public function handle_exam_paper_upload()
    {
        ...
        $priority_papers = isset($_POST['priority_papers']) && is_array($_POST['priority_papers'])
            ? array_filter(array_map('intval', $_POST['priority_papers']))
            : array();
        ...
        if (!empty($_FILES['exam_file']['name'])) {
            ...
            if ($uploaded_file && !isset($uploaded_file['error'])) {
                ...
                if (!empty($priority_papers)) {
                    ...
                    // New paper gets highest priority (position 1)
                    $new_paper_priority = $max_priority + count($priority_papers) + 1;

                    // Update priorities for selected papers in order
                    foreach ($priority_papers as $index => $paper_id) {
                        if ($paper_id > 0) {
                            // Position 2, 3, 4, etc.
                            $paper_priority = $max_priority + count($priority_papers) - $index;

                            $wpdb->update(
                                $table_name,
                                array('priority_order' => $paper_priority),
                                array('id' => $paper_id),
                                array('%d'),
                                array('%d')
                            );
                    ...
                    $priority_order = $new_paper_priority;
                } else {
                ...
                if ($result) {
                    ...
                    if (!empty($priority_papers)) {
                        $message .= ' Priority order has been set for ' . count($priority_papers) . ' paper(s).';
                    }
